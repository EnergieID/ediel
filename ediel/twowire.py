...
from .misc import date_range, open_filename, sort_mixed_list
...
class TwoWireParser(UNIBaseParser):
    """Base class for parsing TwoWire files."""
    ...
    def val_range(self) -> Tuple[int, Optional[int]]:
        """Get the range of values in the file."""
        raise NotImplementedError

    @property
    def meta_range(self) -> Tuple[Optional[int], int]:
        """Get the range of metadata in the file."""
        raise NotImplementedError
...
class TwoWireAMRParser(TwoWireParser):
    ...
    def get_timeseries_frame(self, allow_duplicate_names: bool = True) -> pd.DataFrame:
        """Get the timeseries frame."""
        df = super().get_timeseries_frame(allow_duplicate_names=allow_duplicate_names)
